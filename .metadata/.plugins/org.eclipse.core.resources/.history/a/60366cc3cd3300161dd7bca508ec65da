...
public class TAGProject {
	...
	public ArrayList<Student> getStudentsPerClassroom(Connection connection, String classroom_id) throws Exception {
		try {
			PreparedStatement ps = connection
					.prepareStatement("SELECT * FROM student_enrollment, student_identification "
	...
	/*
	 * Metódo para retorno de uma classe específica cadastrada no TAG: Ele ira
	 * fazer uma query do banco, através da clausula SELECT. Nesse metódo irá
	 * retornar uma classe através do seu INEP_ID
	 */
	public ArrayList<Classroom> getDisciplinesByClassID(Connection connection, String id) throws Exception {
		try {
			PreparedStatement ps = connection
					.prepareStatement("SELECT * FROM classroom WHERE ID = '" + id + "'");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				Classroom classroom = new Classroom();
				classroom.setRegister_type(rs.getString("register_type"));
