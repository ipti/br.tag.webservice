...
public class TAGProject {
	...
	public ArrayList<Credentials> getCredentials(Connection connection, String username, String password) throws Exception {
		try {
			String passwordEncrypted = getPasswordEncrypted(password);
			PreparedStatement ps = connection.prepareStatement("SELECT id, name, instructor_fk, username, password, type FROM users "
	...
	public ArrayList<Student> getChildrenPerParent(Connection connection, String username) throws Exception {
		try {
			PreparedStatement ps = connection
					.prepareStatement("SELECT DISTINCT student_identification.*, edcenso_city.*, edcenso_uf.*,"
							+ " edcenso_nation.*, student_enrollment.student_fk, student_enrollment.id"
							+ " FROM student_identification, users, edcenso_city, edcenso_uf, edcenso_nation, student_enrollment"
			...
			while (rs.next()) {
				...
				student.setEnrollment_fk(rs.getString("student_enrollment.id"));
	...
	public ArrayList<Student> getStudents(Connection connection, String inep_id) throws Exception {
		try {
			PreparedStatement ps = connection
					.prepareStatement("SELECT DISTINCT * FROM student_enrollment, student_identification, edcenso_city, edcenso_uf, edcenso_nation"
	...
	public ArrayList<School> getSchoolsByUserFK(Connection connection, String user_fk) throws Exception {
		try {
			PreparedStatement ps = connection
					.prepareStatement("SELECT school_identification.* FROM school_identification, users_school"
							+ " WHERE users_school.user_fk = '" + user_fk
							+ "' AND school_identification.inep_id = users_school.school_fk;");
	...
	public ArrayList<Grade> getGrades(Connection connection) throws Exception {
		try {
			...
			while(rs.next()) {
				...
				grade.setGrade1(rs.getString("grade1"));
				grade.setGrade2(rs.getString("grade2"));
				grade.setGrade3(rs.getString("grade3"));
				grade.setGrade4(rs.getString("grade4"));
				grade.setRecovery_grade1(rs.getString("recovery_grade1"));
				grade.setRecovery_grade2(rs.getString("recovery_grade2"));
